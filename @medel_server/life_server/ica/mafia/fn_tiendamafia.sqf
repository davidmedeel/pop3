...
_tiendaAvispas = [
	["----- Avispas Africanas -----", 0],
	["medel_avispas_uniforme", 2500 * Desc],
	["medel_avispas_chaleco", 8000 * Desc],
	["medel_avispas_boina", 1000 * Desc],
	["golf_gti_avispas", 50000 * Desc]
];
...
if (_param isEqualTo "menu") exitWith {
	...
	_bd = call mafia;
	_nm = group player getVariable "gang_name";

	if (_bd == 0) then { _arrayTienda append _tiendaLVL0 };
	if (_nm == "Avispas Africanas") then { _arrayTienda append _tiendaAvispas };
...
if (_param isEqualTo "compro") exitWith {
	...
	if (_cosa isEqualto "medel_avispas_uniforme") exitWith { ["avispas", _cosa, _precio] spawn ica_fnc_comproRopa; };
