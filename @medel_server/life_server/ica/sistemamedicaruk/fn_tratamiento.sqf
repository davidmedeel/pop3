...
switch (_cosa) do {
	...
	case "pop_csi": {
		...
		if !(_zona == "pecho") exitWith {hint "La identificación se debe hacer en el pecho"; ocupado = nil};
		if (player == _victima) exitWith {hint "No puedes aplicarte ese tratamiento a ti mismo"; ocupado = nil};
		...
		_nombreReal = _victima getVariable "realname";
		_nombreFalsificado = _victima getVariable "nombreFake";
		hint parsetext format ["<t color='#DE2828'><t size='2'><t align='center'>INFORMACIÓN DEL PACIENTE<br/><t color='#21618C'><t size='1.2'><t align='center'>Nombre real: %1<br/>Nombre falsificado: %2</t>", _nombreReal, _nombreFalsificado];
	};
};
