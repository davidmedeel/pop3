...
_query = switch(_side) do {
	...
	case civilian: {_returnCount = 15; format["GetCivilianPlayer:%1",_uid];};
...
switch (_side) do {
	...
	case civilian: {
		...
		_queryResult set[16, (missionNamespace getVariable[format["houses_%1",_uid],[]])];
		//Gang data
		_gangData = _uid spawn TON_fnc_queryPlayerGang;
		waitUntil{scriptDone _gangData};
		_queryResult set[17, (missionNamespace getVariable[format["gang_%1",_uid],[]])];
	};
};
