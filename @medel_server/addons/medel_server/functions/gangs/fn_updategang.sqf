...
switch (_mode) do {
	case 0: {
		...
		_query = format["SetBankMaxmembersOwnerGangById:%1:%2:%3:%4",_bank,_maxMembers,_owner,_groupID];
	};
	
	case 1: {
		_query = format["SetBankGangById:%1:%2",([(_group getVariable ["gang_bank",0])] call DB_fnc_numberSafe),_groupID];
	};
	
	case 2: {
		_query = format["SetMaxmembersGangById:%1:%2",(_group getVariable ["gang_maxMembers",8]),_groupID];
	};
	
	case 3: {
		_owner = _group getVariable["gang_owner",""];
		if(_owner == "") exitWith {};
		_query = format["SetOwnerGangById:%1:%2",_owner,_groupID];
	};
	...
	case 4: {
		...
		_query = format["SetMembersGangById:%1:%2",_membersFinal,_groupID];
	};
};
