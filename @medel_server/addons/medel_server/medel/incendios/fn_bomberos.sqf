...
if (_param isEqualTo "NPC") exitWith {
    ...
    _npc addAction["<t size='1.5' color='#DE2828'>Empezar servicio de Bombero<t\>",{["vestirseBombero"] spawn ica_fnc_bomberos;},[],1.4,true,true,"","(playerside isEqualTo independent)",5,false,"",""];
	_npc addAction["<t size='1.5' color='#DE2828'>Terminar servicio de Bombero<t\>",{["bomberoEnd"] spawn ica_fnc_bomberos;},[],1.5,true,true,"","(playerside isEqualTo independent)",5,false,"",""];
	_npc addAction["<t size='1.5' color='#DE2828'>Solicitar Camión de Bomberos<t\>",{["solicito"] spawn ica_fnc_bomberos;},[],1.7,true,true,"","(playerside isEqualTo independent)",5,false,"",""];
	_npc addAction["<t size='1.5' color='#DE2828'>Entregar Camión de Bomberos<t\>",{["entregar"] spawn ica_fnc_bomberos;},[],1.8,true,true,"","(playerside isEqualTo independent)",5,false,"",""];
};

if (_param isEqualTo "vestirseBombero") exitWith {
...
if (_param isEqualTo "solicito") exitWith {
    ...
    _camion = "mercedes_atego_bomberos_ingame" createVehicle getMarkerPos "correos_car1";
	_camion allowdamage false;
	_camion setDir 106;
    _camion setvariable ["incendios",0,true];
    _camion setvariable ["propietario",name player,true];
	_camion addAction["<t size='1.5' color='#DE2828'>Disparar agua<t\>",{[_this select 0,_this select 0,3] spawn ica_fnc_tiroAgua;},[],1.8,true,true,"","!(vehicle player isEqualTo player)",5,false,"",""];
	sleep 2;
