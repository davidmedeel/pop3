...
hint "Has enseñado tu DNI falsificado";
...

{
	if(isPlayer _x && (player distance _x) < 5) then {
		if( _x != _player) then {
			[_player, ([_badgeNumber, (count _badgeNumber) - 7, (count _badgeNumber) - 1] call _substring)] remoteexeccall ["life_fnc_seedniFalso", _x];
		};
	};
