...
for "_i" from 1 to _doors do {
	_selPos = _b selectionPosition format["Door%1_Button",_i];
...
if(_b animationPhase format["door%1",_door] == 0) then {
	_b animate[format["door%1",_door],1];
} else {
	_b animate[format["door%1",_door],0];
};
